...
//function to put the products
function prod(products) {
    productList.innerHTML = ''; // Clear the previous products
    products.forEach(([title, color, description, price, imageSrc, id]) => {
        ...
        productdiv.innerHTML = `
            ...
            <h2>${title} ${color}</h2>
            <p>${description}
            <a href="item.html" class="readmore" onclick="viewProduct(${id}, '${title}', '${color}', '${description}', '${price}', '${imageSrc}')">Read more</a></p>
            <div class="price">${price}</div>
            <button onclick="buyitem('${title}', '${color}', '${price}', '${imageSrc}')" class="buybut">Buy</button>
        `;
        productList.appendChild(productdiv);
    });
}

// Set event listeners for category links
document.getElementById("hoodies-link").addEventListener("click", () => {
    prod(hoodies);
});

document.getElementById("jumpers-link").addEventListener("click", () => {
    prod(jumpers);
});

document.getElementById("tshirts-link").addEventListener("click", () => {
    prod(tshirts);
});

//function for the buy item button
function buyitem(title, color, price, imageSrc) {
    const cart=JSON.parse(localStorage.getItem("cart")) || [];
    const newItem={title,color,price,imageSrc};
    cart.push(newItem);
    localStorage.setItem("cart", JSON.stringify(cart));
    alert(`${title} (${color}) has been added to the cart`);
}

//function to view the product
function viewProduct(id, title, color, description, price, imageSrc) {
    // Create an object to store the product details
    const selectedProduct = { id, title, color, description, price, imageSrc };
